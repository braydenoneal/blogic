...
import com.braydenoneal.blang.wrapper.codec.statement.StatementType
...
object ValueCodecs {
    val FUNCT_CODEC: Codec<Funct> = RecordCodecBuilder.create {
        it.group(
            ...
            Codec.list(Codec.pair(Codec.STRING, ExpressionType.CODEC)).fieldOf("defaultParameters").forGetter(Funct::defaultParameters),
            Codec.list(StatementType.CODEC).fieldOf("statements").forGetter(Funct::statements)
        ).apply(it, ::Funct)
    }
    ...
    val STRUCT_VALUE_CODEC: MapCodec<StructValue> = RecordCodecBuilder.mapCodec {
        it.group(
            Codec.list(Codec.pair(Codec.STRING, ValueType.CODEC)).fieldOf("value").forGetter(StructValue::value)
