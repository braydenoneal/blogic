...
object BuiltinExpression {
    fun parse(program: Program, name: String): Expression {
        return when (name) {
            ...
            "blockItem" -> BlockItemBuiltin(Arguments.parse(program))
            "tag" -> TagBuiltin(Arguments.parse(program))
            "tags" -> TagsBuiltin(Arguments.parse(program))
            ...
            "readItemCount" -> ReadItemCountBuiltin(Arguments.parse(program))
