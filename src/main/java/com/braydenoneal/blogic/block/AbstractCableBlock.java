...
public abstract class AbstractCableBlock extends Block {
	...
	public BlockState getStateWithUpdatedShape(BlockState state, WorldAccess world, BlockPos pos) {
		...
		for (Direction direction : Direction.values()) {
			...
			if (neighborEmitsRedstonePower
				&& (
					(neighborState.getBlock() instanceof AbstractCableBlock && canConnectToCableOfState(neighborState))
					|| !(neighborState.getBlock() instanceof AbstractCableBlock)
				)
			) {
				setTrue = true;
				connectedSides++;
			...
			newState = newState.with(DIRECTION_BOOLEAN_PROPERTY_MAP.get(direction), setTrue);
		}
	...
	public abstract boolean canConnectToCableOfState(BlockState state);
	...
	public static class CableGroup {
		...
		public void addToCableGroupAt(WorldAccess world, BlockPos pos) {
			...
			for (Direction direction : Direction.values()) {
				...
				if (!cableGroup.contains(neighborPos) && state.get(DIRECTION_BOOLEAN_PROPERTY_MAP.get(direction))) {
					if (neighborState.getBlock() instanceof AbstractCableBlock && neighborState.get(DIRECTION_BOOLEAN_PROPERTY_MAP.get(direction.getOpposite()))) {
